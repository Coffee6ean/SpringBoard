...
a new array with all the values in the array passed to the function 
doubled
...
function extractFullName(arr){
    let newArr = [];
    arr.forEach((element) => {
        newArr.push(element["first"] + " " + element["last"]);
    });
...
function filterByValue(arr, key) {
    let newArr = [];
    arr.filter((element) => {
        if(element[key] !== undefined) newArr.push(element);
    });
...
and filter to double and then filter the odd numbers).
